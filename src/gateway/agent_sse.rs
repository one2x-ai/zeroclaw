use super::AppState;
use crate::agent::loop_::{build_shell_policy_instructions, build_tool_instructions};
use crate::agent::loop_::{run_tool_call_loop, trim_history_for_gateway};
use crate::agent::loop_::{scrub_credentials, DRAFT_CLEAR_SENTINEL, DRAFT_PROGRESS_SENTINEL};
...
pub async fn handle_agent_sse(
    ...
    if history.is_empty() {
        ...
        if !state.provider.supports_native_tools() {
            prompt.push_str(&build_tool_instructions(state.tools_registry_exec.as_ref()));
        }
        prompt.push_str(&build_shell_policy_instructions(&config_guard.autonomy));
    ...
    let user_message = if !had_prior_history {
        ...
        let memory_context = crate::channels::recall_memory_context(
            state.mem.as_ref(),
            &message,
            min_relevance_score,
        )
    ...
    tokio::spawn(async move {
        let mem_for_save = Some(state_for_task.mem.clone());
        let (approval_manager, provider_label, max_history) = {
            let config_guard = state_for_task.config.lock();
            (
        ...
        let tools_registry = Arc::clone(&state_for_task.tools_registry_exec);
        ...
        let result = run_tool_call_loop(
            ...
            "agent_sse",
            &state_for_task.multimodal,
            state_for_task.max_tool_iterations,
            None,
            Some(delta_tx),
            None,
            &[],
        ...
        trim_history_for_gateway(&mut history, max_history);
        if let Some(mem) = mem_for_save.as_ref() {
            let history_key = format!("session:{}:history", session_id_for_task);
            let to_store: Vec<&ChatMessage> =
